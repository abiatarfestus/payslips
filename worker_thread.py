...
class Worker(QObject):
    ...
    def __init__(self, month, office, payslips_path, email):
        ...
        self.parent_dir = os.path.dirname(
            self.payslips_path
        )  # Directory where the payslips file is originally located
        self.path = os.path.join(self.parent_dir, self.directory).replace(
            "\\", "/"
        )  # Path to where new payslip files will be saved (parent_dir + directory)
